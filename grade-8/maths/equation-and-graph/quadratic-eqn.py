...
class QuadraticIntro(Scene):
    ...
    def construct(self):
        ...
        self.wait(0.5)
        self.play(
            FadeIn(b1, scale=0.5), FadeIn(times),
        ...
        ).arrange(DOWN, buff=0.3).scale(0.9).to_corner(DL, buff=0.8)
        ...
        self.play(FadeOut(title), FadeOut(riddle), FadeOut(check),
                  FadeOut(flashback), FadeOut(banner))
        self.wait(0.3)

        # ==================================================================
        # BEAT 3 — The squaring machine: sign annihilation (~45 s)
        # ==================================================================
        mach_box = RoundedRectangle(corner_radius=0.25, width=3.4, height=2.0,
                                    color=BLACK)
        ...
        def feed_simple(n):
            ...
            self.play(inp.animate.move_to(
                mach_box.get_left() + RIGHT * 0.4).scale(0.7), run_time=0.8)
        ...
        self.play(FadeIn(inp, shift=RIGHT * 0.5), run_time=0.6)
        self.play(inp.animate.move_to(
            mach_box.get_left() + RIGHT * 0.5).scale(0.7), run_time=0.8)
        calc = MathTex("(", "-", "3", ")", r"\times", "(", "-", "3", ")",
                       color=BLUE)
        calc.move_to(mach_box.get_center() + DOWN * 0.15)
        ...
        self.play(m1.animate.move_to(mid + LEFT * 0.12),
                  m2.animate.move_to(mid + RIGHT * 0.12), run_time=1.5)
        self.play(Flash(mid, color=PURPLE, flash_radius=0.5),
                  FadeOut(m1), FadeOut(m2))
        cancel_cap = Text("minus minus becomes plus!", font_size=26,
                          color=PURPLE)
        cancel_cap.next_to(mach_box, DOWN, buff=0.5)
        self.play(FadeIn(cancel_cap))
        rest = VGroup(calc[0], calc[2], calc[3], calc[4], calc[5],
                      calc[7], calc[8])
        ...
        answers = VGroup(
            ...
            MathTex(r"-3\ \checkmark", color=GREEN).scale(1.2),
        ).arrange(RIGHT, buff=1.4).next_to(machine, UP, buff=0.8)
        two_txt = Text("Two different inputs  →  the same output 9",
                       font_size=26, color=BLACK).to_edge(DOWN, buff=0.8)
        self.play(FadeIn(answers[0], shift=UP * 0.3),
                  FadeIn(answers[1], shift=UP * 0.3))
        self.play(FadeIn(two_txt))
        self.wait(2)

        self.play(FadeOut(answers), FadeOut(two_txt),
                  FadeOut(cancel_cap), FadeOut(out9))
        self.play(machine.animate.scale(0.5).to_corner(UL, buff=0.5),
                  run_time=1.5)
        ...
        def add_row(n, i, extra_anims=None):
            row = MathTex(str(n), r"\;\rightarrow\;", str(n * n),
                          color=BLACK).scale(0.85)
        ...
        # ---- PREDICT-THEN-REVEAL ----
        prompt = Text("Now the negatives: -1, -2, -3.\nWhere will the dots land?",
                      font_size=28, color=PURPLE, line_spacing=1)
        ...
        for j, n in enumerate([-1, -2, -3]):
            ...
            mirror = DashedLine(axes.c2p(n, n * n), axes.c2p(-n, n * n),
        ...
        ).arrange(DOWN, buff=0.15).to_edge(UP, buff=0.5).shift(RIGHT * 2)
        self.play(FadeIn(curve_label))
        self.wait(2)

        self.play(FadeOut(machine), FadeOut(header),
                  FadeOut(rows), FadeOut(curve_label))
        ...
        caption1 = Text("we can also write it this way", font_size=24,
                        color=GRAY)
        ...
        hline = always_redraw(lambda: Line(
            axes.c2p(-4, h.get_value()), axes.c2p(4, h.get_value()),
        ...
        self.play(FadeIn(sol_l, scale=3), FadeIn(sol_r, scale=3),
                  Flash(axes.c2p(-3, 9), color=GREEN),
                  Flash(axes.c2p(3, 9), color=GREEN))
        drop_l = DashedLine(axes.c2p(-3, 9), axes.c2p(-3, 0),
                            color=GREEN, stroke_width=3)
        drop_r = DashedLine(axes.c2p(3, 9), axes.c2p(3, 0),
                            color=GREEN, stroke_width=3)
        ...
        self.play(TransformMatchingTex(eq3, eq4), FadeOut(caption1),
                  run_time=1.5)
        caption2 = Text("same question, different way of writing it",
        ...
        self.play(FadeOut(eq4), FadeOut(caption2),
                  FadeOut(hline), FadeOut(sol_l), FadeOut(sol_r),
                  FadeOut(drop_l), FadeOut(drop_r), FadeOut(lab_l),
                  FadeOut(lab_r), FadeOut(axes), FadeOut(curve),
                  FadeOut(dots_pts))
        self.wait(0.3)
        ...
        # ==================================================================
        def_title = Text("QUADRATIC EQUATION", font_size=42, color=BLACK,
                         weight=BOLD)
        ...
        tag2 = VGroup(MathTex(r"\times", color=RED),
                      Text("no x² — means it has only one solution",
                           font_size=24, color=GRAY)
                      ).arrange(RIGHT, buff=0.2).next_to(ex2, RIGHT, buff=0.8)
        ...
        self.play(FadeOut(def_title), FadeOut(def_body), FadeOut(ring),
                  FadeOut(examples), FadeOut(tag1), FadeOut(tag2),
                  FadeOut(tag3), FadeOut(punch))
        self.wait(0.3)
        ...
        # ==================================================================
        axesL = Axes(
            x_range=[-1, 5, 1], y_range=[-2, 12, 2],
            x_length=4.4, y_length=4.6,
        ...
        targL = Line(axesL.c2p(-1, 9), axesL.c2p(5, 9),
                     color=PURPLE, stroke_width=3)
        ...
        lineR = always_redraw(lambda: Line(
            axesR.c2p(-4, hr.get_value()), axesR.c2p(4, hr.get_value()),
            color=PURPLE, stroke_width=3))
        ...
        self.play(Flash(axesR.c2p(-3, 9), color=GREEN),
                  Flash(axesR.c2p(3, 9), color=GREEN),
                  FadeIn(labR))
        ...
        self.play(*[FadeOut(m) for m in [
            axesL, lineL, targL, dotL, labL,
            axesR, curveR, lineR, dotsR, labR, solLab_l, solLab_r]],
            run_time=2)
        self.wait(1)