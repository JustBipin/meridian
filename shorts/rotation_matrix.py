...
class RotationMatrices3D(Scene):
    def setup(self):
        ...
        super().setup()
        apply_theme(manim_scene=self, theme_name="onedark")
    ...
    def construct(self):
        ...
        def get_formula_tex():
            ...
            return MathTex(
                ...
                r" = " + mat_str + r"\begin{bmatrix} x \\ y \\ z \end{bmatrix}",
                font_size=28,
            )
        ...
        # =============================================================
        # 9. GROWING CIRCULAR ROTATION ARROW
        # =============================================================

        rotation_arc = VMobject()

        # rotation_arrowhead = Triangle(
        #     color="#FFD166",
        #     fill_color="#FFD166",
        #     fill_opacity=1.0,
        #     stroke_width=0,
        # ).scale(0.15)
        rotation_arrowhead = Dot(color=YELLOW)
        ...
        def update_rotation_arrow(mob):
            ...
            # ---------------------------------------------------------
            # No visible arc at theta = 0
            # ---------------------------------------------------------

            if th < 0.015:
                rotation_arc.clear_points()

                rotation_arrowhead.set_opacity(0)

                return

            # ---------------------------------------------------------
            # Clamp to one complete revolution
            # ---------------------------------------------------------

            th = min(th, 2 * np.pi)

            # ---------------------------------------------------------
            # Generate growing circular arc
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # Position arrowhead at current endpoint
            # ---------------------------------------------------------
            ...
            if len(points) >= 5:
                ...
                if tangent_norm > 1e-8:
                    ...
                    rotation_arrowhead.move_to(end)

                    rotation_arrowhead.set_angle(angle - np.pi / 2)

                    rotation_arrowhead.set_opacity(1)
        ...
        rotation_arc.add_updater(lambda m: None)
        ...
        # Add the rotation arrow BEFORE the cube
        # so the cube visually sits on top of the arrow.
