...
class HelixArcLength(ThreeDShortScene):
    """
    Helix Arc Length Short (Under 45s)
    -----------------------------------
    TOP    -> title (disappears) -> pinned reference formula + active goal
    MIDDLE -> hero object: the helix (rotates in place, camera stays fixed)
    BOTTOM -> active derivation only, old step slides up 50% opacity,
              new step derives below it, bottom 30% kept empty
    """
    ...
    def construct(self):
        ...
        self.set_camera_orientation(phi=70 * DEGREES, theta=-60 * DEGREES, zoom=1)

        # ---------------------------------------------------------
        # TOP ZONE: title -> pinned reference formula + goal
        # ---------------------------------------------------------
        ...
        with self.voiceover(
            text="represented by the equation, -r- of -t- equals, cosine -t-, sine -t-, comma -t-."
        ) as tracker:
            self.play(FadeOut(title), Write(ref_formula), run_time=tracker.duration)
        self.play(FadeIn(goal, shift=UP * 0.1))

        # ---------------------------------------------------------
        # MIDDLE ZONE: hero object — the helix
        # ---------------------------------------------------------
        ...
        # rotate the HERO SECTION only, never the camera, so the fixed
        # formulas above and below stay facing the viewer
        self.play(Rotate(hero, angle=PI / 6, axis=UP, about_point=hero.get_center()), run_time=1.2)

        # ---------------------------------------------------------
        # a point on the helix + its position vector
        # ---------------------------------------------------------
        t0 = 2.4
        ...
        with self.voiceover(
            text="Suppose you have a point on the helix, and its position vector is, -r- of -t-."
        ) as tracker:
            self.play(FadeIn(dot), Create(pos_vec), run_time=tracker.duration)
        ...
        # small displacement dl along the curve
        t1 = t0 + 0.35
        ...
        # ---------------------------------------------------------
        # BOTTOM ZONE: active derivation, one line at a time
        # ---------------------------------------------------------
        ...
        with self.voiceover(
            text=(
                "But speed is just the magnitude of velocity, and velocity is just "
                "the first derivative of position, -r- prime."
            )
        ) as tracker:
            self.play_step(step3, active=step2, old=step1, run_time=tracker.duration)

        # only the tangent direction matters now -> fade the other hero
        # annotations to 50% and bring in the tangent vector
        tangent_vec = Arrow3D(
            start=p_point,
        ...
        with self.voiceover(text="which is, negative sine -t-, cosine -t-, comma one.") as tracker:
            self.play_step(
                step4,
                active=step3,
                old=step2,
        ...
        step5a = MathTex(
            r"|\vec{r}\,'(t)| = \sqrt{\sin^2 t + \cos^2 t + 1^2}",
            font_size=26,
        )
        with self.voiceover(
            text="So the speed is just, square root of, sine squared -t-, plus cosine squared -t-, plus one squared."
        ) as tracker:
        ...
        with self.voiceover(
            text="Put the new value of speed into the -d-l- formula: -d-l- equals, square root of two, -d-t-."
        ) as tracker:
        ...
        with self.voiceover(text="Put in the definite values, from zero to four pi, and integrate.") as tracker:
            self.play_step(step7, active=step6, old=step5a, run_time=tracker.duration)

        # final answer -> replaces the active derivation line
        ...
        with self.voiceover(text="and you get, four pi root two.") as tracker:
            self.resolve_step(step7, answer, fade_out=step6, run_time=tracker.duration)

        # goal at the top resolves too
